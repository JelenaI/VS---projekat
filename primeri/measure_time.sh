...
KLEE=~/klee/klee/Release+Asserts/bin/klee
STAT=~/klee/klee/Release+Asserts/bin/klee-stats
SEARCH=""
LIMIT=0
VERSION=""
KLEE_FLAGS="-exit-on-error"
...
if  [ $# -lt 2 ]; then
	...
	SEARCH=$1
	if [ $SEARCH != "dfs" ]  && [ $SEARCH != "bfs" ] &&  [ $SEARCH != "nas" ] && [ $SEARCH != "random-path" ] && [ $SEARCH != "random-state" ]  ; then
	...
	if [ $2 == "yes" ] ; then
		KLEE_FLAGS=$KLEE_FLAGS" --write-paths --write-pcs"
	fi
	if [ $# -gt 2 ] ; then
	  LIMIT=$3 
	fi
